...
var joinRoomCommandHandler = {
  ...
  fn: function fn(room, command) {
    ...
    var newUser = {
      id: command.payload.userId || (0, _uuid.v4)(), // client can cache/store userId and send it already with "joinRoom" command
...
exports.default = joinRoomCommandHandler;
...
function handleNewRoom(room, command, newUser) {
  ...
  var joinedRoomEventPayload = {
    userId: newUser.id,
    users: _defineProperty({}, newUser.id, newUser)
  };
...
function handleExistingRoom(room, newUser) {
  ...
  if (matchingExistingUser) {
    ...
    newUser = matchingExistingUser.set('username', newUser.username || matchingExistingUser.get('username')).set('disconnected', false).toJS();
  }
